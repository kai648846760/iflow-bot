...
class StdioACPClient:
    ...
    @staticmethod
    def _extract_result_text_payload(payload: Any) -> str:
        """Extract text from prompt result payload when chunk updates are absent."""
        seen: set[int] = set()

        def visit(node: Any, *, semantic: bool = False) -> list[str]:
            if node is None:
                return []
            node_id = id(node)
            ...
            if isinstance(node, str):
                text = node.strip()
                return [text] if semantic and text else []
            if isinstance(node, list):
                parts: list[str] = []
                for item in node:
                    parts.extend(visit(item, semantic=semantic))
                return parts
            ...
            if node_type in {"text", "markdown"}:
                text_value = node.get("text")
                if isinstance(text_value, str) and text_value.strip():
                    parts.append(text_value.strip())

            for key in ("text", "content", "contents", "output", "outputs", "message", "messages", "item", "items", "part", "parts"):
                if key == "text" and node_type in {"text", "markdown"}:
                    continue
                if key in node:
                    parts.extend(visit(node.get(key), semantic=True))
            return parts

        return "\n".join(part for part in visit(payload) if part).strip()
    ...
    async def prompt(
        ...
        try:
            ...
            if not response.content and "error" not in final_response:
                response.content = self._extract_result_text_payload(final_response.get("result", {}))
...
class StdioACPAdapter:
    ...
    def __init__(
        ...
        self._auth_retry_cooldown_seconds = 1800.0
...
TOOLS.md - Your Tools（你的工具）定义了你可以使用的工具列表，包括每个工具的名称、功能描述、使用方法等, 每次学会一个工具，你便要主动更新该文件。"""
        ...
        try:
            ...
            def _strip_transport_wrappers(text: str) -> str:
                ...
                if not content:
                    return ""
                if "用户消息:" in content:
                    idx = content.find("用户消息:") + len("用户消息:")
                    return content[idx:].strip()
                for marker in ("[/language]", "[/message_source]"):
            ...
            for chat in chat_history:
                ...
                if role == "user":
                    content = _strip_transport_wrappers(full_text)
                    if not content:
                        continue
    ...
    async def connect(self) -> None:
        ...
        for attempt in range(2):
            ...
            try:
                ...
                if (
                    self._last_auth_succeeded is False
    ...
    async def chat_stream(
        ...
        if response.error:
            ...
            if "terminated" in error_text and (stream_content or "").strip():
                logger.warning("Chat stream returned terminated with content, returning partial content")
            elif "tool" in error_text and "failed" in error_text and (stream_content or "").strip():
                logger.warning("Chat stream tool failed with content, returning partial content")
            else:
                raise StdioACPError(f"Chat error: {response.error}")
